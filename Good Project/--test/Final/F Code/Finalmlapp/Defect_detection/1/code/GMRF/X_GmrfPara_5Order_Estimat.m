function EstiPara_Gmrf=X_GmrfPara_5Order_Estimat(L,Maxgraylevel)
%L:input gray image
%Maxgraylevel:the bit depth of the input image
%output features: 12 dimension
%  Gmrf四阶模型的参数估计
%
% Order    Gmrf的阶次
% 邻域结构为：
...
%
L=double(L);
s=0;  G=zeros((size(L,1)-4)*(size(L,2)-4),12);
for i=3:size(L,1)-2
     for j=3:size(L,2)-2
       s=s+1;
       G(s,:)=X_Gmrf_ParaG(L(i-2:i+2,j-2:j+2),5);
       X(s)=L(i,j);
   end
end
